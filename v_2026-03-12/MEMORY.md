...
| 2026-03-08 | 跳过审计环节：UI 完成后直接准备启动开发 | **正确流程：** UI 完成 → PM 确认 → FE 确认 → OCA 审计 → 状态变更 → 启动开发 |
| 2026-03-11 | 沟通闭环缺失：通知 QA 未确认收到，导致测试延迟 50 分钟 | **改进：** 重要通知必须"收到回复"，未回复时主动跟进；建立项目状态看板 |
| 2026-03-11 | 设备环境感知缺失：未确认用户设备（手机端无法操作 CLI） | **改进：** 任务开始前增加"环境检查"步骤（手机/电脑/网络） |
| 2026-03-11 | 流程执行松懈：PM 直接向 OCA 汇报，违反 TEAM.md 章程 | **改进：** 严格执行 TEAM.md，关键门禁不妥协 |
| 2026-03-11 | 风险上报延迟：等待用户询问才跟进 QA 进度 | **改进：** 建立"风险 15 分钟上报"机制，超时自动升级 |
